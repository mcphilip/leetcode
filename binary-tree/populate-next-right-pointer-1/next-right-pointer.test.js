...
describe('treeToArrayWithNextPointer', () => {
    it('should return correct array for leetcode example tree', () => {
        const tree = setupTree([1,2,3,4,5,6,7]);
        ...
        tree.left.right.next = tree.right.left;
        tree.right.left.next = tree.right.right;

        expect(treeToArrayWithNextPointer(tree)).toEqual([1,'#',2,3,'#',4,5,6,7,'#']);
    });
});
...
describe('connect', () => {
    ...
    it('should return correct serialized tree for leetcode example tree', () => {
        ...
        expect(treeToArrayWithNextPointer(wiredUpTree)).toEqual([1,'#',2,3,'#',4,5,6,7,'#']);
    })
})
